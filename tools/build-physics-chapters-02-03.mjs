...
const renderScript = path.join(root, "tools", "render_physics_chapters_02_03.py");
const workDir = path.join(root, "outputs", "physics-101-chapters-02-03");
const targetSeconds = 205;
const voiceModel = "gpt-4o-mini-tts-2025-12-15";
const voiceName = "coral";

const visualTargets = {
  "track-mystery": ["tile-lane", "grass-lane", "carpet-lane", "prediction-cover"],
  "run-comparison": ["launchers", "balls", "stopping-traces"],
  "motion-changes": ["start-strip", "speed-up-strip", "slow-down-strip", "stop-strip", "turn-strip"],
  "before-after": ["time-markers", "position-gaps", "stop-marker"],
  "kick-ended": ["launcher", "ball", "contact-arrow", "motion-trail"],
  "surface-friction": ["ball-surface-contact", "friction-arrow", "distance-rails"],
  "fair-surface-test": ["ball-lock", "launch-lock", "start-lock", "surface-card", "repeat-dots"],
  "distance-data": ["distance-table", "column-graph", "surface-lanes"],
  "stored-kick": ["stored-kick-capsule", "contact-timeline", "friction-arrow", "corrected-claim"],
  "direction-change": ["incoming-path", "bumper-contact", "outgoing-path"],
  "surface-prediction": ["prediction-rails", "rubber-mat", "test-next-stamp"],
  "motion-routine": ["compare-step", "change-step", "pair-step", "evidence-step"],
  "support-mystery": ["book", "foam", "contact-patch", "prediction-marker"],
  "support-pair": ["book-on-foam-arrow", "foam-on-book-arrow", "support-label"],
  "still-forces": ["supported-state", "falling-state", "restored-state", "misconception-cross"],
  "support-load": ["second-book", "compression-ruler", "support-arrow"],
  "applied-push": ["robot-hand", "cart", "contact-ring", "push-arrow"],
  "applied-pull": ["rubber-band", "cart", "pull-arrow", "stored-force-cross"],
  "comparison-meter": ["resting-length", "stretch-ruler", "two-centimetres", "four-centimetres", "scope-note"],
  "fair-pull-test": ["cart-lock", "surface-lock", "start-lock", "time-lock", "two-trials"],
  "pull-data": ["result-table", "column-graph", "relative-arrows", "evidence-stamp"],
  "silent-table": ["claim-card", "table", "foam", "book", "contact-evidence"],
  "choose-model": ["one-arrow-model", "no-arrow-model", "paired-arrow-model", "contact-patch"],
  "force-routine": ["pair-step", "contact-step", "receiver-step", "direction-step", "fair-test-step"],
};
...
async function main() {
  ...
  const chapterNumbers = requested ? [requested] : [2, 3];
  if (chapterNumbers.some((number) => ![2, 3].includes(number))) {
    throw new Error("--chapter must be 2 or 3.");
  }
...
async function buildChapter(course, chapter) {
  ...
  const sceneName = chapter.number === 2 ? "PhysicsChapter02VoiceDirected" : "PhysicsChapter03VoiceDirected";
  await run(python, ["-m", "manim", "--media_dir", chapterWorkDir, "--disable_caching", "-r", "1280,720", "--fps", "24", "-o", silentName, renderScript, sceneName], {
  ...
  await run(ffmpeg, ["-y", "-i", silentVideo, "-i", masterWav, "-map", "0:v:0", "-map", "1:a:0", "-c:v", "libx264", "-preset", "slow", "-crf", "21", "-pix_fmt", "yuv420p", "-c:a", "aac", "-b:a", "160k", "-shortest", "-movflags", "+faststart", videoPath]);
...
function teacherInstructions(chapterNumber, sectionId) {
  const focus = chapterNumber === 2
    ? "Make before, after, contact ended, friction and evidence distinct. Never sound as if motion needs a stored forward force."
    : "Make book on foam, foam on book, support force, applied force and relative comparison distinct. Say not Newtons with calm precision.";
  return [
    ...
    "Use a natural middle pitch, lively but unforced rhythm, settled factual endings and genuine pauses after prediction questions.",
    "Aim for approximately 140 to 155 words per minute. Pronounce all object-pair force labels crisply.",
  ].join(" ");
}
...
function buildVtt(cues) {
  ...
  cues.forEach((cue) => {
    ...
    chunks.forEach((chunk, localIndex) => {
      ...
      const targets = visualTargets[cue.id];
      if (!targets) throw new Error(`Missing visual targets for ${cue.id}.`);
      captionCues.push({
        index: captionIndex,
        start: round(cursor),
        end: round(end),
