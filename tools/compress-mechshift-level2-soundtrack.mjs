...
try {
  ...
  await page.goto(`${baseUrl}/mechshift-rescue/level-2/`, { waitUntil: "domcontentloaded" });
  const encoded = await page.evaluate(async () => {
    const mimeType = "audio/webm;codecs=opus";
    if (!MediaRecorder.isTypeSupported(mimeType)) throw new Error(`${mimeType} is not supported by this browser.`);
    const response = await fetch("../assets/level-2/audio/stormrail-command-loop.wav");
  ...
  const output = resolve("mechshift-rescue/assets/level-2/audio/stormrail-command-loop.webm");
